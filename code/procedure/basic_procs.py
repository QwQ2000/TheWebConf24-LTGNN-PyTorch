...
def BPR_train_original(dataset, recommend_model, loss_class, epoch, neg_k=1, w=None):
    ...
    #print(torch.unique(posItems).shape, torch.unique(negItems).shape)
    #input(torch.unique(torch.cat([posItems, negItems])).shape)
...
def Test_accelerated(dataset, Recmodel, epoch, w=None, multicore=0):
    u_batch_size = world.config['test_u_batch_size']
    dataset: utils.BasicDataset
    testDict: dict = dataset.testDict
    Recmodel: model.LightGCN
    ...
    with torch.no_grad():
        ...
        for batch_users in utils.minibatch(users, batch_size=u_batch_size):
            ...
            rating = Recmodel.f(torch.matmul(users_emb, all_items.t()))
            #rating = rating.cpu()
            exclude_index = []
            exclude_items = []
            for range_i, items in enumerate(allPos):
